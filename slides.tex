...
\begin{frame}
	\frametitle{\textcolor{KITblue}{Vorstellung}}
	\begin{itemize}
		\item Sarah Lutteropp
		\item 19 Jahre, 3. Semester Informatik Bachelor
		\item There will be cake \ldots
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{Organisatorisches - Zum Übungsbetrieb}}
	\begin{itemize}
		\item \textbf{Abgabe:} \textcolor{KITblue}{Handschriftlich} in Zweiergruppen.
		\item \textbf{Schein:} 
		\begin{itemize}
			\item Klausurbonus
			\item Ein Notenschritt
			\item Ab 50\% der erreichbaren Punkte
		\end{itemize}
		\item Tutoriumsmaterial und aktueller Punktestand online.
		\begin{itemize}
			\item \url{http://tgdi.de/}
...
\begin{frame}
	\frametitle{\textcolor{KITblue}{Organisatorisches - Zum Tutorium}}
	\begin{itemize}
		\item Stoff soll wiederholt werden
		\item Dabei Fokus auf Übungsbetrieb
		\item Fragen/Vorschläge/Anmerkungen willkommen!
		\item Folien basieren auf Tut-Folien von Simon Stroh und Moritz v. Looz im WS 2010/2011
...
\begin{frame}
	\frametitle{\textcolor{KITblue}{Kurze Wiederholung: Formale Sprachen}}
	Eine \textcolor{KITblue}{Formale Sprache} $L$ ist eine Teilmenge aller Wörter über einem endlichen Alphabet $\Sigma$. Also $L \subseteq \Sigma^*$\\[0.3cm]
	Beispiele:
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Wiederholung der Wiederholung: Reguläre Sprachen}}
 \vspace{-1cm}
...
\(\Sigma = \{a,b\}\)
	\KITframe[Yeah, I think I'll stick to gray as a background color. Thanks again!]
		{\parbox{\textwidth}{\begin{itemize}
		\item \(L_1 = \{w| w \in \Sigma^*, \mbox{w besteht aus einer geraden Anzahl $a$s}\)\}
		\item \(L_2 =\{w| w \in \Sigma^*, \mbox{w enthält gleich viele $a$s und $b$s.}\)\}		
		\end{itemize}}
	}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Deterministische endliche Automaten}}
...
\begin{frame}
	\begin{enumerate}
	\setcounter{enumi}{0}
	\item Aufgabe: Konstruiere einen DEA, der alle durch 4 teilbaren Zahlen akzeptiert. Als Eingabe erhält der Automat dabei die Zahl in ihrer binären Darstellung. Also ist $\Sigma = \{0, 1\}$. Z.B. soll der Automat $8_{10} = 1000_{2}$ akzeptieren, aber $7_{10} = 111_{2}$ ablehnen.
	\pause \\[10pt]
	Tipp: Restklassen als Zustände modellieren (Kommentar: Es geht auch mit 3)
	\end{enumerate}
\end{frame}
\begin{frame}
	\frametitle{\textcolor{KITblue}{DEA: Lösung}}
	\begin{enumerate}
	\setcounter{enumi}{0}
	\item Idee
		\begin{itemize}
		...
		\item $\Sigma = \{0,1\}$
		\item $\delta(q_n, c) \rightarrow q_{n \cdot 2 + c\mbox{ mod }4}$ mit $c\in\Sigma$\\
		\item $S = q_0$\\
		\item $F = \{q_0\}$
		\end{itemize}
	\end{enumerate}
...
\begin{frame}
	\frametitle{\textcolor{KITblue}{NEA: Beispiel}}
	\begin{figure}
	\begin{tikzpicture}
	...
	\draw[->] (-1,0) -- (q_0);
	\draw[->] (q_0) -- (q_1) node[midway,anchor=south] {$y$};
	\draw[->] (q_1) -- (q_2) node[midway,anchor=south] {$x$};
	\draw (q_1) edge [loop above] node {$y$, $x$} (q_1);
	\end{tikzpicture}
	\end{figure}
	\pause Bei Eingabe von $x$ im Zustand $q_1$ gibt es mehrere Möglichkeiten.
\end{frame}

\begin{frame}
\frametitle{\textcolor{KITblue}{Nichtdeterministische endliche Automaten}}
        ...
        \item $F$:   Endzustandsmenge $\subseteq Q$
        \end{itemize}
	Damit der NEA ein Wort akzeptiert, muss es \textcolor{KITblue}{einen} akzeptierenden Weg geben.
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{NEA: Aufgabe}}
...
\frame{
\frametitle{\textcolor{KITblue}{Potenzmengenkonstruktion}}
...
\begin{frame}
  \frametitle{\textcolor{KITblue}{NEA2DEA: Aufgaben}}
...
\begin{frame}
	\frametitle{\textcolor{KITblue}{Eliminierung von $\varepsilon$-Übergängen}}
...
%Pumpin' Lemma
\begin{frame}
\frametitle{\textcolor{KITblue}{Pumping-Lemma}}
...
\end{frame}
\begin{frame}
\frametitle{\textcolor{KITblue}{Pumping-Lemma: Übersicht}}
\begin{itemize}
\item Für jede reguläre Sprache gilt das Pumping-Lemma, nicht jede Sprache für die das Pumping-Lemma gilt ist regulär!
\item In der Übung wird üblicherweise die Kontraposition des Pumping-Lemmas verwendet. Man zeigt für eine Sprache, dass das Pumping-Lemma \textcolor{KITblue}{nicht} erfüllt ist, und daraus folgt, dass diese Sprache \textcolor{KITblue}{nicht} regulär sein kann.
\begin{itemize}
\item Konkret: Finden wir für \textcolor{KITblue}{jedes} $n \in \mathbb{N}$ \textcolor{KITblue}{ein} $w \in L$ mit $\left|w\right| > n$, so dass für \textcolor{KITblue}{jede} Darstellung $w = uvx$ mit $v \neq \varepsilon$ sowie $\left|uv\right| \leq n$ \textcolor{KITblue}{ein} $i \in \mathbb{N}_0$ existiert mit $uv^ix \notin L$, dann ist $L$ nicht regulär.
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Beispiel}}
...
\item Nun ist aber für \textcolor{KITblue}{jede} Darstellung $w = uvx$ mit $\left|uv\right| \leq n$ und $v \neq \varepsilon$ $v = a^m$ mit $m > 0$. Demnach ist $uv^0x = a^lb^n \notin L$, da $l < n$.
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Pumping-Lemma: Aufgaben}}
...
\item $L_1 = \{ww^R | w \in \{a,b\}^*\}$, wobei $w^R$ das
        `Spiegelwort' zu $w$ ist (Sprache der Palindrome
        gerader Länge)
...
\frametitle{\textcolor{KITblue}{DEA: Aufgabe}}
\begin{enumerate}
\setcounter{enumi}{0}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Konstruktion eines RA aus einem DEA}}
...
\begin{itemize}
\item Es ist \textcolor{KITblue}{$L = \bigcup\limits_{f\in F} L_{s,n,f}$}
\item Es ist weiterhin \textcolor{KITblue}{$L_{q_r,i+1,q_t} = L_{q_r,i,q_t} \cup (L_{q_r,i,q_{i+1}}(L_{q_{i+1},i,q_{i+1}})^*L_{q_{i+1},i,q_t})$}
...
\end{frame}
\begin{frame}
\frametitle{\textcolor{KITblue}{Beispiele zum Verständnis}}
\vspace{-1cm}
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[scale=0.6,node distance=1.9cm,shorten >=1pt,auto]
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Ausführliche Konstruktion}}
\vspace{-1cm}
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[scale=0.6,node distance=1.9cm,shorten >=1pt,auto]
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Aufgabe}}
...
\begin{frame}
	\frametitle{\textcolor{KITblue}{DEA: Zusatzaufgabe}}
	\begin{enumerate}
	\item 
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Minimierung von DEAs}}
...
\end{frame}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Methodischer Ansatz}}
...
\end{frame}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Äquivalenz}}
 \vspace{-1cm}
 ...
 \begin{block}{Definition (Äquivalenz):}
  Zwei Zustände $p$ und $q$ eines deterministischen endlichen Automaten heißen \textcolor{KITblue}{äquivalent} ($p \equiv $q),
...
\end{frame}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Äquivalenzklassenautomat}}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Aufgabe}}
 Konstruieren Sie zu folgendem DEA den Äquivalenzklassenautomaten:
...
\subsection{Erstes Übungsblatt}
\begin{frame}
	\frametitle{\textcolor{KITblue}{1. Übungsblatt - Statistik}}
	\begin{itemize}
		\item 15 von 20 Leuten haben Bearbeitungen abgegeben
		\item davon durchschnittliche Punktzahl: $\approx 16.917 \text{ von } 30  \hat{\approx} 56.39 \%$ der Gesamtpunktzahl
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{1. Übungsblatt - Aufgabe 1}}
	\begin{itemize}
		\item formale Fehler
		\item "$\subseteq$" und "$\supseteq$" zeigen
		\item $A$ regulärer Ausdruck $\Rightarrow w \in L(A), \text{ nicht } w \in A$
		\item d) Distributivgesetz $\rightarrow$ bitte auch anwenden
		\item \textbf{Entweder}: Aussage widerlegen durch Gegenbeispiel \textbf{oder} für den allgemeinen Fall zeigen
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{1. Übungsblatt - Aufgabe 2}}
	\begin{itemize}
		\item Jede Sprache besteht nur aus Wörtern, die Teil einer regulären Sprache sind, da $\Sigma^*$ regulär.
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{1. Übungsblatt - Aufgabe 3}}
	\begin{itemize}
		\item Das $\epsilon$ nicht "wegoptimieren"!
		\item Nur die $L_{q_r,i,q_t}$ ausrechnen, die ihr auch braucht... (rekursiv einsetzen)
		\item Ausdrücke rechtzeitig vereinfachen
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{1. Übungsblatt - Aufgabe 4}}
	\begin{itemize}
		\item Der Zustand selbst gehört mit zu seinem $\epsilon$-Abschluss
		\item Automat ohne $\epsilon$-Übergänge $\leadsto$ nochmal üben?
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{1. Übungsblatt - Aufgabe 5}}
	\begin{itemize}
		\item Bei $\{s\}$ starten, nur die Mengen hinzunehmen, die man auch rauskriegt
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{1. Übungsblatt - Aufgabe 6}}
	\begin{itemize}
		\item wenig Probleme
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{\textcolor{KITblue}{1. Übungsblatt - Aufgabe 7}}
	\begin{itemize}
		\item $n$ \textbf{beliebig}, $w$ aussuchen, für \textbf{alle} gültigen Zerlegungen argumentieren
		\item $\leadsto$ Pumping-Lemma wiederholen?
	\end{itemize}
\end{frame}

\subsection{Nerode-Relation}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Nerode-Relation}}
 \begin{block}{Definition 2.25}
 Für eine Sprache \(L \subseteq \Sigma^*\) ist die \textcolor{KITblue}{Nerode-Relation} \(R_L\) definiert durch:
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Aufgaben zur Nerode-Relation}}
Geben Sie für die folgenden Sprachen jeweils die Äquivalenzklassen bezüglich der Nerode-Relation $R_L$ an. Welche der Sprachen sind regulär? Begründen Sie ihre Antwort. Konstruieren Sie für diese Sprachen jeweils den Minimalautomaten.
\begin{itemize}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Turingmaschine}}
...
Die Turingmaschine besteht aus einem beidseitig \textcolor{KITblue}{unendlichen} Eingabe- und Rechenband
mit einem freibeweglichen Lese-/Schreibkopf, der von einer \textcolor{KITblue}{endlichen} Kontrolle gesteuert wird. 
\end{block}
\end{frame}
\begin{frame}
\vspace{-1cm}
...
\frame{
	\frametitle{\textcolor{KITblue}{Graph einer Turingmaschine}}
	\begin{center}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Konfigurationen}}
Oft wird die Situation, in der sich eine Turingmaschine $\mathcal{M} =(Q,\Sigma,\Gamma,\delta,s,F)$ befindet, durch die Angabe der
\emph{Konfiguration} in der Form \textcolor{KITblue}{$w(q)av$} codiert, wobei $w,v \in \Gamma^*, a
...
\begin{center}
 ...
 Welche Konfigurationen durchläuft die Maschine bei Eingabe des
  Wortes $01010$?
...
\begin{center}
 ...
 Wie verändert $\mathcal{M}$ die Eingabe?
\end{frame}

\begin{frame}
\frametitle{\textcolor{KITblue}{Aufgabe}}
Entwerfen Sie eine deterministische Turing-Maschine $\mathcal M := (Q, \Sigma, \Gamma, \delta, s, F)$, die genau die folgende Sprache akzeptiert: $$L:=\{ww^R \mid w \in \{a, b\}^*\}$$ wobei $w^R$ das "`Spiegelwort"'~zu $w$ ist.
Dies ist die Sprache der Palindrome gerader Länge. Beschreiben Sie zunächst kurz in Worten, wie Ihre Turing-Maschine arbeitet.
...
\Gamma = \Sigma \cup \{\sqcup\}$.  Insbesondere ist also $\sqcup$ das
Blanksymbol.
...
\begin{frame}
	...
	\end{center}
	\tiny{Quelle: \url{www.cs.utah.edu/~draperg/cartoons/2005/turing.png}}
\end{frame}

\subsection{Zweites Übungsblatt}
\begin{frame}
	\frametitle{\textcolor{KITblue}{2. Übungsblatt - Statistik}}
	\begin{itemize}
		\item 12 von 20 Leuten haben Bearbeitungen abgegeben
		\item davon durchschnittliche Punktzahl: $\approx 11.58 \text{ von } 15  \hat{\approx} 77.22 \%$ der Gesamtpunktzahl
	\end{itemize}
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{Erinnerung - Zum Übungsbetrieb}}
	\begin{itemize}
		\item \textbf{Abgabe:} Handschriftlich in Zweiergruppen.
		\item \textbf{Schein:} 
		\begin{itemize}
			\item \textcolor{KITblue}{Klausurbonus}
			\item \textcolor{KITblue}{Ein Notenschritt}
			\item Ab \textcolor{KITblue}{50\%} der erreichbaren Punkte
		\end{itemize}
		\item Tutoriumsmaterial und aktueller Punktestand online.
		\begin{itemize}
			\item \url{http://tgdi.de/}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Wiederholung: Nerode-Relation}}
 \begin{block}{Definition 2.25}
 Für eine Sprache \(L \subseteq \Sigma^*\) ist die \textcolor{KITblue}{Nerode-Relation} \(R_L\) definiert durch:
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Aufgabe zur Nerode-Relation}}

Sei $\Sigma = \{a\}$.
Zeigen Sie mit Hilfe der Nerode-Relation die Nichtregularität der Sprache $L$:

$L := \{w \in \Sigma^*\mid w = a^{n^2}, n \in \mathbb{N}\}.$
\end{frame}

\subsection{Entscheidbarkeit}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Definitionen zur TM (Vorlesung)}}
 \begin{enumerate}
  \item Eine TM \textcolor{KITblue}{akzeptiert} eine Eingabe $w \in \Sigma^*$, wenn sie nach Lesen von $w$ in einem Zustand aus $F$ stoppt.
  \item Sie \textcolor{KITblue}{akzeptiert} eine Sprache $L \subseteq \Sigma^*$ genau dann, wenn sie ausschließlich Wörter $w$ aus $L$ als Eingabe akzeptiert.
  \pause
  \item Eine Sprache $L \subseteq \Sigma^*$ heißt \textcolor{KITblue}{rekursiv} oder \textcolor{KITblue}{entscheidbar}, wenn es eine Turingmaschine gibt, die auf allen Eingaben stoppt und
	ein Wort $w \in \Sigma^*$ genau dann akzeptiert, wenn $w \in L$ gilt.
	\pause
  \item Eine Sprache $L \subseteq \Sigma^*$ heißt \textcolor{KITblue}{rekursiv-aufzählbar} oder \textcolor{KITblue}{semi-entscheidbar}, wenn es eine Turingmaschine gibt, 
	...
	\pause
  \item Eine TM \textcolor{KITblue}{realisiert} die Funktion $f: \Sigma^* \rightarrow \Gamma^*$ mit $$f(w) = \begin{cases} \text{Ausgabe der TM nach Abarbeitung von } w & \text{wenn die TM hält} \\ \text{undefiniert} & \text{sonst} \end{cases}$$
 \end{enumerate}
\end{frame}

\begin{frame}
 \frametitle{\textcolor{KITblue}{Beispiel zur Akzeptanz}}
 \begin{itemize}
  \item $\Sigma := \{a, b\}$  
  \item $F := \{q_4\}$
 \end{itemize}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Sprache}}
 \begin{itemize}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Universelle Turingmaschine}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Universelle Sprache}}
Die Universelle Sprache $L_u$ über $\{0,1\}$ ist definiert durch:
\newline
$L_u := \{wv \mid v \in L_{T_w}\}$

\begin{block}{Satz}
	$L_u$ ist unentscheidbar (siehe Skript).
\end{block}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Halteproblem}}
Das Halteproblem definiert die folgende Sprache:
\newline
$\mathcal{H} := \{wv \mid T_w \text{ hält auf Eingabe } v\}$

\begin{block}{Satz}
	$\mathcal{H}$ ist unentscheidbar (siehe Skript).
\end{block}

\begin{block}{Anmerkung}
	$\mathcal{H}$ ist semi-entscheidbar.
\end{block}
\end{frame}

\begin{frame}
 \frametitle{\textcolor{KITblue}{Diagonalsprache}}
 Die Diagonalsprache $L_d$ ist definiert durch:
 \newline
 $L_d := \{w_i \mid M_i \text{ akzeptiert } w_i \text{ nicht }\}$ 

	\begin{block}{Satz}
		$L_d$ und $L_d^c$ sind unentscheidbar (siehe Skript).
	\end{block}
	\begin{block}{Aufgabe}
 		Zeigen Sie, dass die Diagonalsprache $L_d$ nicht semi-entscheidbar ist.
 	\end{block}
\end{frame}

\begin{frame}
\frametitle{\textcolor{KITblue}{Aufgabe zur Entscheidbarkeit}}

Zeigen Sie, dass die Sprache $L_\emptyset := \{ <\mathcal{M}> \mid \mathcal{M}  \text{ Turingmaschine }, L(M) = \emptyset\}$ nicht entscheidbar ist.
\end{frame}

\subsection{Erweiterungen von Turingmaschinen}
\begin{frame}
\frametitle{\textcolor{KITblue}{Erweiterungen von Turingmaschinen}}
...
% Mehrkopf
\begin{frame}
\frametitle{\textcolor{KITblue}{Mehrkopf-Turingmaschinen}}
\vspace{-2cm}
\begin{figure}[H]
\begin{center}
...
% Mehrband
\begin{frame}
\frametitle{\textcolor{KITblue}{Mehrband-Turingmaschinen}}
\vspace{-2cm}
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[scale=0.3]
...
% Mehrdim
\begin{frame}
\frametitle{\textcolor{KITblue}{Mehrdimensionale Turingmaschinen}}
\vspace{-2cm}
\begin{figure}[H]
\begin{center}
\begin{tikzpicture}[scale=0.3]
...
\subsection{Wieder semi-entscheidbar}

\begin{frame}
	\frametitle{\textcolor{KITblue}{Erinnerung}}
	\begin{block}{Semientscheidbarkeit}
 Eine Sprache $L \subseteq \Sigma^*$ heißt \textcolor{KITblue}{rekursiv-aufzählbar} oder \textcolor{KITblue}{semi-entscheidbar}, wenn es eine Turingmaschine gibt, 
	die genau $w$ genau dann akzeptiert, wenn $w \in L$ gilt. Das Verhalten der Turingmaschine für Eingaben $w \not\in L$ ist damit nicht definiert.
	Sie stoppt entweder nicht in einem Endzustand oder aber stoppt gar nicht.
	\end{block}
\end{frame}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Aufgabe zur Semi-Entscheidbarkeit}}

\begin{block}{Erinnerung}
	$L_\emptyset := \{ <\mathcal{M}> \mid \mathcal{M}  \text{ Turingmaschine }, L(M) = \emptyset\}$
\end{block}
\begin{block}{Aufgabe}
Zeigen Sie, dass die Komplementsprache $L_\emptyset^c$ zu $L_\emptyset$ semi-entscheidbar ist.
\end{block}
\end{frame}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Postsches Korrespondenzproblem}}
...
Eine nicht-leere Folge $I = i_1, i_2, \ldots, i_m$ von Indizes $\{1, \ldots, n\}$ heißt Lösung zu $P$, wenn $x_{i_1}x_{i_2}\ldots{}x_{i_m} = y_{i_1}y_{i_2}\ldots{}y_{i_m}$.
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Aufgabe}}
Finde eine Lösung für folgendes PKP
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Satz von Rice}}
...
$$ C_S = \{ i \mid \text{ die von $M_i$ berechnete Funktion liegt in $S$} \} $$
unentscheidbar. 
\end{block}
\end{frame}
% Beispiel
...
\begin{frame}
	\frametitle{\textcolor{KITblue}{Bis zum nächsten Mal}}
	\vspace{-1cm}
	\begin{center}
	\includegraphics[width=0.75\textwidth]{images/virus.pdf}
	\end{center}
	\tiny{Quelle: \url{http://www.semiconfundive.de/comics/2008-01-20.png}}
\end{frame}

\section{Wdh Entscheidbarkeit}
\begin{frame}
	\frametitle{\textcolor{KITblue}{Wiederholung Entscheidbarkeit}}
	\textcolor{blue}{Wann ist eine Sprache $L$ entscheidbar?}
	\pause
	Eine Sprache $L \subseteq \Sigma^*$ heißt \textcolor{KITblue}{rekursiv} oder \textcolor{KITblue}{entscheidbar}, wenn es eine Turingmaschine gibt, die auf allen Eingaben stoppt und ein Wort $w \in \Sigma^*$ genau dann akzeptiert, wenn $w \in L$ gilt.
\end{frame}

\begin{frame}
	\frametitle{\textcolor{KITblue}{Klausuraufgabe zur Entscheidbarkeit}}
	Sei $L$ eine nicht entscheidbare Sprache \"uber dem Alphabet $\Sigma=\{0,1\}$ und sei $(01)^k \not\in L$ f\"ur jedes $k\in \mathbb N_{>0}$.
Zeigen Sie: Die Sprache \[L' = \{w_1\#w_2\mid (w_1 \in L \wedge w_2 \not\in L) \text{ oder } (w_1 \not\in L \wedge w_2 \in L) \}\]
\"uber dem Alphabet $\Sigma'=\{0, 1, \#\}$ ist nicht entscheidbar.
\end{frame}

\section{Probleme}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Probleme}}
 \begin{block}{Definition (Skript)}
 Ein \textcolor{KITblue}{Problem $\Pi$} ist gegeben durch:
 \begin{enumerate}
 ...
 \end{enumerate}
 \end{block}
 Ein \textcolor{KITblue}{Problembeispiel} \textit{I} \textcolor{KITblue}{(Instanz)} von $\Pi$ erhalten wir, indem wir die Parameter von $\Pi$ festlegen.
\end{frame}

\begin{frame}
 \frametitle{\textcolor{KITblue}{Entscheidungsproblem}}
 \begin{block}{Definition}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Optimierungsproblem und Entscheidungsproblem}}
 \begin{block}{Beispiel}
  \begin{enumerate}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Optimierungsproblem}}
 Gegeben sei ein ungerichteter Graph $G=(V,E)$ mit $V:=\{1,\ldots,n\}$ und $E\subseteq \{\{u,v\} \mid u,v \in V, u\neq v\}$. Weiter seien $c:E \rightarrow \mathbb{R}^+$ eine Kostenfunktion auf den Kanten des Graphen sowie $s\in V$ und $t\in V$ zwei ausgewiesene Knoten. 
Ein $s$-$t$-Schnitt $(S,T)$ in $G$ ist eine Partitionierung $(S,T)$ von $V$ in zwei Partitionen mit $s\in S$ und $t \in T$. Es gilt $S \cup T = V$ und $S\cap T = \emptyset$.
Die Kosten c$(S,T)$ eines Schnittes $(S,T)$ seien definiert als die Summe der Kosten aller Kanten, deren eines Ende in $S$ und deren anderes Ende in $T$ liegt: 
\[c(S,T) := \sum_{\{u,v\} \in E \text{ : } u\in S, v \in T}c(\{u,v\})\]
%
Betrachten Sie die Menge der $s$-$t$-Schnitte in $G$ und formulieren Sie ein naheliegendes Optimierungsproblem. 
Formulieren Sie ein zu Ihrem Optimierungsproblem geh\"orendes Optimalwert-- und Entscheidungsproblem.
\end{frame}

\subsection{Kodierungsschemata}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Kodierungsschemata (VL)}}
 \begin{block}{Definition Kodierungsschema}
 Ein \textcolor{KITblue}{Kodierungsschema} s ordnet jedem Problembeispiel eines Problems eine Zeichenkette oder Kodierung über einem Alphabet $\Sigma$ zu. 
 ...
 \begin{block}{Äquivalenz}
  Zwei Kodierungsschemata $s_1$, $s_2$ heißen \textcolor{KITblue}{äquivalent} bezüglich eines Problems $\Pi$, falls es Polynome p1, p2 gibt, so dass gilt:
  \[
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Kodierungsschemata}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Kodierung von TUT}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{TUT}}
\begin{block}{Beispiel}
 Gegeben sei eine Instanz \textit{I} von TUT mit 2 Tutorien, 5 Studenten, einer maximalen Teilnehmerzahl von 3 und folgenden Wahlen:
 \begin{enumerate}
  \item Student 1 wählt Tutorium 2
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Die Klasse $\mathcal{P}$}}
...
%\begin{frame}
%\frametitle{\textcolor{KITblue}{Sind folgende Sprachen in $\mathcal{P}$?}}
%\begin{itemize}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Laufzeitbetrachtung}}
In dieser Aufgabe sowie in Aufgabe 4 bezeichne $s_1$ die un\"are Kodierung und $s_k$, $k >1$, die $k$-\"are Kodierung.
Das Entscheidungprobelm \textit{PRIMES} fragt ob eine gegeben Zahl $q \in \mathbb N$ eine Primzahl ist.
\begin{enumerate}
\item Betrachten Sie $L_1 := L[\text{\textit{PRIMES}},s_1]$. Ein naiver Algorithmus f\"ur \textit{PRIMES} k\"onnte alle Zahlen $2,3,\ldots,p-1$ darauf hin \"uberpr\"ufen, ob sie die gegebene Zahl $p$ teilen. Beschreiben Sie kurz in Worten die Arbeitsweise einer \textbf{deterministischen} Turing-Maschine, die diesen Algorithmus implementiert und damit $L_1$ entscheidet. Geben Sie die Laufzeit Ihrer Turing-Maschine asymptotisch an. Ist $L_1$ in $\mathcal P$? Begr\"unden Sie gegebenenfalls, warum $L_1$ in $\mathcal P$ ist.
\item Erst 2002 konnte gezeigt werden, dass das Problem \textit{PRIMES} in der Klasse $\mathcal P$ liegt. Weshalb folgt \textit{PRIMES} $\in \mathcal P$ nicht aus a)?
\end{enumerate}
\end{frame}

\begin{frame}
\frametitle{\textcolor{KITblue}{Aufgabe}}
Sei $\Pi$ ein Enscheidungsproblem \"uber der Menge der nat\"urlichen Zahlen (d.h. Instanzen des Problems k\"onnen als nat\"urliche Zahl kodiert werden). Zeigen Sie, dass $s_1$ und $s_2$  bez\"uglich $\Pi$ nicht \"aquivalent sind.
\end{frame}

\subsection{Nichtdeterministische Turingmaschinen}
\begin{frame}
\frametitle{\textcolor{KITblue}{Nichtdeterministische Turingmaschinen}}
...
Wir definieren eine nichtdeterministische Turingmaschine als Erweiterung einer deterministischen.
 Wir erweiteren die TM um ein \textcolor{KITblue}{Orakelmodul}, das bevor der deterministischen Berechnung Zeichen auf das Band links von der Eingabe schreibt,
...
\end{frame}
\begin{frame}
\frametitle{\textcolor{KITblue}{Nichtdeterministische Turingmaschinen: Anmerkung}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{$\mathcal{NP}$}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Grundidee: $\mathcal{NP}$-Entscheider}}
...
\end{enumerate}
Man spricht daher auch von den \textcolor{KITblue}{effizient verifizierbaren} Entscheidungsproblemen.
\end{frame}
...
\begin{frame}
	...
	\vspace{-1cm}
	\begin{center}
	\includegraphics[width=0.75\textwidth]{images/iloveyou.pdf}
	\end{center}
	\tiny{Quelle: \url{http://www.semiconfundive.de}}
\end{frame}

% Unentscheidbarkeit universelle Sprache
\begin{frame}
\frametitle{\textcolor{KITblue}{Unentscheidbarkeit ohne Rice}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Polyreduktionen}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{$\mathcal{NP}$-Schwere}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Transitivität von Polyreduktionen und $\mathcal{NP}$}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{$\mathcal{NP}$-Vollständigkeit}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{CLIQUE}}
\begin{block}{Problem}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{CLIQUE Beispiel}}
\vspace{-1cm}
\begin{figure}[H]
...
\begin{frame}
\frametitle{\textcolor{KITblue}{$\mathcal{NP}$-Vollständigkeitsbeweis}}
\begin{itemize}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Allgemeines zu $NP$-Vollständigkeitsbeweise}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{SAT}}
\begin{block}{Problem}
\textbf{Gegeben:}
\begin{itemize}
...
\end{block}
\begin{itemize}
\item \textcolor{KITblue}{Das} Standardproblem.
...
\end{frame}
\begin{frame}
\frametitle{\textcolor{KITblue}{3-SAT}}
\begin{block}{Problem}
\textbf{Gegeben:}
\begin{itemize}
 \item Menge $U$ von Variablen
 \item Menge $C$ von Klauseln über $U$, jede Klausel enthält genau \textcolor{KITblue}{drei} Literale
\end{itemize}
\textbf{Frage:} Existiert eine erfüllende Wahrheitsbelegung von $C$?
\end{block}
\begin{itemize}
...
\end{frame}
\begin{frame}
\frametitle{\textcolor{KITblue}{MAX2SAT}}
\begin{block}{Problem}
\textbf{Gegeben:}
\begin{itemize}
 \item Menge $U$ von Variablen
...
\end{frame}
\begin{frame}
\frametitle{\textcolor{KITblue}{COLOR}}
\begin{block}{Problem}
...
\end{frame}
\begin{frame}
\frametitle{\textcolor{KITblue}{EXACT COVER}}
\begin{block}{Problem}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Sudoku}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{SUBSET-SUM}}
\begin{block}{Problem}
\textbf{Gegeben:}
\begin{itemize}
...
%% - PARTITION
\begin{frame}
\frametitle{\textcolor{KITblue}{PARTITION}}
\begin{block}{Problem}
...
\end{frame}
\begin{frame}
\frametitle{\textcolor{KITblue}{Und viele mehr!}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{BIN-PACKING}}
\begin{block}{Problem}
\textbf{Gegeben:}
\begin{itemize}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{KNAPSACK}}
 Gegeben:(M, w, c, W, C)
 \begin{itemize}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{KNAPSACK - Beispiel}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Komplementsprachen}}
\begin{block}{Definition}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{co- und so...}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{NPI}}
\begin{block}{Definition}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Pseudopolynomielle Algorithmen}}
\begin{block}{Definition}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Orakelturingmaschinen}}
\begin{block}{Definition}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Beispiele}}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Suchprobleme}}
 \begin{block}{Definition}
  Ein Suchproblem $\Pi$ wird beschrieben durch
  \begin{itemize}
   \item die Menge der Problembeispiele oder Instanzen $D_\Pi$
   \item für $I\in D_\Pi$ die Menge $S_\Pi(I)$ \textcolor{KITblue}{aller} Lösungen von $I$.
  \end{itemize}
 \end{block}
\begin{block}{Lösung}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Suchprobleme als Relationen}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Turingreduzierbarkeit}}
\begin{block}{Definition}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{$\mathcal{NP}$-Schwere von Suchproblemen}}
 \begin{block}{Definition}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{CLIQUE als Suchproblem}}
 \begin{block}{Aufgabe}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Aufzählungsprobleme}}
 \begin{block}{Definition}
  Ein \textcolor{KITblue}{Aufzählungsproblem} $\Pi$ ist gegeben durch
  \begin{itemize}
  ...
  \end{block}
  \begin{block}{Lösung}  
  Die \textcolor{KITblue}{Lösung} der Instanz $I$ eines Aufzählungsproblem $\Pi$ besteht in der Angabe der Kardinalität $|S_\Pi(I)|$ von $\Pi$.
 \end{block}
\end{frame}

 \begin{frame}
 \frametitle{\textcolor{KITblue}{CLIQUE als Aufzählungsproblem}}
  \begin{block}{Aufgabe}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{INTEGER PROGRAMMING}}
...
Ein \textcolor{KITblue}{DOMINATING SET} von $G$ ist eine Teilmenge $C \subseteq V$, so dass jeder Knoten $v$ entweder selbst in $C$ ist oder zu einem Knoten $w$ in $C$ adjazent ist. 
Gibt es ein Vertex Cover von $G$, das höchstens $k$ Knoten enthält?

Formulieren Sie DOMINATING SET als \textcolor{KITblue}{Integer Program} 
...
\frame{
\frametitle{\textcolor{KITblue}{Grammatiken}}
\begin{block}{Definition}
...
\frame{
\frametitle{\textcolor{KITblue}{Chomsky-Hierarchie}}
...
\frame{
\frametitle{\textcolor{KITblue}{CH-0}}
\begin{itemize}
...
\frame{
\frametitle{\textcolor{KITblue}{CH-1}}
...
\frame{
\frametitle{\textcolor{KITblue}{CH-2}}
...
\frame{
\frametitle{\textcolor{KITblue}{CH-3}}
\begin{itemize}
...
\frame{
\frametitle{\textcolor{KITblue}{Chomsky-Normalform}}
...
\frame{
\frametitle{\textcolor{KITblue}{Umwandlung in Chomsky-Normalform}}
\begin{enumerate}
...
wird jedes Vorkommen von $\textcolor{red}{a}$ durch ein \textcolor{KITblue}{neues} Nichtterminalsymbol $\textcolor{blue}{A}$ ersetzt
...
vorher \textcolor{KITblue}{rekursiv} durch ihre ``Vorwegnahme'' mit den anderen Produktionen
...
Variablen rechts werden \textcolor{blue}{ \textcolor{KITblue}{neue} Nichterminale} eingeführt
...
\frame{
\frametitle{\textcolor{KITblue}{CYK Überblick}}
...
\frame{
\frametitle{\textcolor{KITblue}{CYK}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Pumping-Lemma für kontextfreie Sprachen}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Ogdens Lemma für kontextfreie Sprachen}}
\begin{exampleblock}{Lemma}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Beweisidee}}
\begin{itemize}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Zu Ogden und Pumping ...}}
\begin{itemize}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Lösungshinweise für Teilaufgabe a}}
...
\end{frame}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Lösungshinweise für Teilaufgabe b} }     
...
\end{frame}
\begin{frame}
\frametitle{\textcolor{KITblue}{Fallunterscheidung für Teilaufgabe b}}
\begin{itemize}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Greibach Normalform}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Zur Greibach-Normalform}}
\begin{itemize}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Umwandlung in Greibach-Normalform}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Annahmen}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Schritt 1}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{$A_i \rightarrow A_i\alpha$}}
Für Regeln der Form 
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Schritt 2}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Aufgabe zur Greibach-Normalform}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Definition}}
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Zu Kellerautomaten}}
\begin{itemize}
\item Akzeptieren wenn \textcolor{KITblue}{entweder} der Stack leer ist, \textcolor{KITblue}{oder} wenn der Automat in einen akzeptierenden Zustand kommt
...
\begin{frame}
\frametitle{\textcolor{KITblue}{Aufgaben}}
\begin{itemize}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Kellerautomat aus Greibach-Normalform}}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Kellerautomat}}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{$\delta$}}
\begin{itemize}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Tripelkonstruktion}}
 \begin{itemize}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Wiederholung Chomsky-Hierarchie}}
...
\begin{frame}
 \frametitle{\textcolor{KITblue}{Anmerkung zum aktuellen Übungsblatt}}
