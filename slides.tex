...
Sei $L$ eine reguläre Sprache, dann existiert eine Zahl $n \in \mathbb{N}$, so dass für jedes Wort $w \in L$ mit $\left|w \right| > n$ eine Darstellung $$w = uvx$$ existiert, sodass folgende Eigenschaften erfüllt sind:
...
\begin{itemize}
\item Konkret: Finden wir für \textcolor{KITblue}{jedes} $n$ \textcolor{KITblue}{ein} $w$ mit $\left|w\right| > n$, so dass für \textcolor{KITblue}{jede} Darstellung $w = uvx$ mit $v \neq \varepsilon$ sowie $\left|uv\right| \leq n$, \textcolor{KITblue}{ein} $i \in \mathbb{N}_0$ mit $uv^ix \notin L$, dann ist $L$ nicht regulär.
...
\begin{itemize}
\item Es ist \textcolor{KITblue}{$L = \cup_{f\in F} L_{s,n,f}$}
...
\end{frame}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Konstruktion der Äquivalenzklassen}}
 \begin{block}{}
  \begin{enumerate}
...
\end{frame}
\begin{frame}
 \frametitle{\textcolor{KITblue}{Beispiel $Q'$}}
 \vspace{-1cm}
 \begin{figure}[h]
\begin{center}
\begin{tikzpicture}[scale=0.5,node distance=1.9cm,shorten >=1pt,auto]
...
\frame{
  \frametitle{\textcolor{KITblue}{Automaten-Minimierung: Beispiel, Methode 1}}
%   \begin{center}
...
\frame{
  \frametitle{\textcolor{KITblue}{Minimierung, Ausführlicheres Beispiel/Aufgabe}}
  \begin{figure}[H]
  \begin{center}
  \begin{tikzpicture}
...
\frame{
  \frametitle{\textcolor{KITblue}{Minimierung, Ausführlicheres Beispiel}}
  \begin{figure}[H]
  \begin{center}
  \begin{tikzpicture}
